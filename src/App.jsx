...
const FISH_DATA = [
  ...
  { id: 2, name: "アユ", nameEn: "Sweetfish (Ayu)", emoji: "🐠", color: "#1a4e7c", accent: "#1a1a14", difficulty: "advanced", flyFriendly: true, flyNote: { ja: "毛鉤を使う「ドブ釣り」は解禁初期の定番。テンカラの逆さ毛鉤にも反応する。", en: "'Dobu-zuri' with wet kebari is a classic early-season method; tenkara sakasa kebari also works." }, season: { ja: "6月解禁〜10月（河川ごとに解禁日あり／落ち鮎期は禁漁）", en: "Opens in June, runs to October (each river sets its own opening day; closed in the spawning run)" }, habitat: { ja: "清流の瀬。石に付いた苔（アカ）を食む縄張りを持つ", en: "Riffles of clear rivers, where fish hold territories over algae-covered stones" }, bestTime: { ja: "日中（水温18〜23℃）・くもりの日。増水後の水色回復時", en: "Daytime at 18–23℃ water, overcast days, as the river clears after a rise" }, description: { ja: "日本の国民的な川魚。縄張り意識を利用した「友釣り」は世界でも珍しい伝統漁法。香りから「香魚」とも呼ばれ、塩焼きが絶品。", en: "Japan's iconic river fish. 'Tomozuri', which exploits its territorial instinct, is a fishing method found almost nowhere else. Famous for its melon-like scent and superb grilled." }, gear: { rod: { ja: "友釣り専用鮎竿 8.5〜9.5m（ドブ釣りは6〜9m）", en: "8.5–9.5m ayu rod (6–9m for dobu-zuri)" }, reel: { ja: "なし（のべ竿）。水中糸とハナカンで組む", en: "None — fixed-line pole with a nose-ring rig" }, line: { ja: "複合メタル0.05〜0.1号 または ナイロン0.15〜0.25号", en: "0.05–0.1 gou composite metal, or 0.15–0.25 nylon" }, hooks: { ja: "イカリ針3本／4本錨 6.5〜7.5号、ハナカン・逆さ針", en: "3- or 4-point anchor hooks #6.5–7.5, plus nose ring and reverse hook" }, lures: ["オトリ鮎（生きた囮）", "アユ用イカリ針", "ドブ釣り毛鉤", "アユイング用ミノー"], tips: { ja: "オトリは弱らせないのが全て。流れの筋に沿って泳がせ、石裏の縄張り鮎の“ナワバリ”へ送り込む。掛かったら竿を立てて空中輸送、すぐに掛かり鮎を次のオトリに交換。", en: "Everything depends on keeping the decoy lively. Swim it along the current seam into the territory behind a stone, then lift both fish clear and swap the fresh one in as your next decoy." } }, howTo: [ { t: { ja: "① 遊漁券を買う", en: "1. Buy the ticket" }, b: { ja: "アユは河川ごとに漁協の管理下。日券（¥2,000〜4,000前後）を漁協・オトリ屋・コンビニで購入。解禁日・禁漁区・使用可能な漁法（友釣り限定区間など）は必ず確認する。", en: "Ayu rivers are co-op managed. Buy a day ticket (about ¥2,000–4,000) from the co-op, a decoy shop or a convenience store, and check the opening date, closed zones and which methods are allowed." } }, { t: { ja: "② オトリ鮎を用意", en: "2. Get a live decoy" }, b: { ja: "川沿いの「オトリ店」で1〜2匹購入（1匹¥600前後）。引舟に入れて川へ。オトリの元気さが釣果を決めるので、日陰・流水で管理する。", en: "Buy one or two live ayu at a riverside decoy shop (about ¥600 each) and carry them in a floating tube. A lively decoy is the whole game — keep it shaded and in flowing water." } }, { t: { ja: "③ 仕掛けをセット", en: "3. Rig the decoy" }, b: { ja: "ハナカンをオトリの鼻に通し、逆さ針を尻ビレ前に打ち、背中側にイカリ針が来るようにセット。水中糸は細いほど泳ぎが良いが切れやすい。", en: "Pass the nose ring through the decoy's nostrils, set the reverse hook ahead of the anal fin and let the anchor hooks trail behind. Thinner line swims better but breaks easier." } }, { t: { ja: "④ 泳がせて縄張りへ", en: "4. Swim it into a territory" }, b: { ja: "アカが付いた石のある瀬へ、流れに逆らわせながらゆっくり泳がせる。1か所で1〜2分反応がなければ次の石へ。野鮎が体当たりでイカリ針に掛かる。", en: "Work riffles with algae-covered stones, letting the decoy swim upstream slowly. Give each stone a minute or two, then move on. The wild fish attacks and hooks itself on the anchor." } }, { t: { ja: "⑤ 取り込み＆オトリ交換", en: "5. Land it, swap it" }, b: { ja: "掛かったら竿を立てて2匹を一気に引き抜き、タモで受ける。掛かった元気な鮎を次のオトリにするのが「循環」。弱ったオトリは引舟へ。", en: "Raise the rod and swing both fish into your net. Use the fresh wild fish as the next decoy — that rotation is what keeps you catching. Rest the tired one in the tube." } } ], spots: [{ name: "長良川（岐阜）", rating: 5.0, type: { ja: "清流", en: "River" } }, { name: "四万十川（高知）", rating: 4.9, type: { ja: "清流", en: "River" } }, { name: "球磨川（熊本）", rating: 4.8, type: { ja: "清流", en: "River" } }, { name: "矢部川・黒木（福岡）", rating: 4.5, type: { ja: "清流", en: "River" } }], regulations: { ja: "遊漁券必須（アユ券は雑魚券と別）。解禁日は6月上旬が多く、産卵期の落ち鮎は禁漁。コロガシ・投網などは許可河川・期間のみ。", en: "Ayu ticket required (separate from the general ticket). Most rivers open in early June and close for the autumn spawning run. Snagging and cast nets are only legal on certain rivers and dates." } },
                      ...
                      <div style={{ background: "#fffdf8", border: "2px solid #0d7377", borderRadius: 12, padding: 14 }}>
                        ...
                        {selectedFish.howTo.map((h, i) => (
                          <div key={i} style={{ marginBottom: i < selectedFish.howTo.length - 1 ? 12 : 0 }}>
                            ...
                            <div style={{ fontSize: "0.86rem", lineHeight: 1.7, color: "#3a3a2a" }}>{(h.b?.[lang] || h.b?.en || h.b?.ja || "")}</div>
                          </div>
                        ))}
                      </div>
                    )}
